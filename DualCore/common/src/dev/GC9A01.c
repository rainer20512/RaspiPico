...
// Column address order, BGR
#define MADCTL_BITORDER         GC9A01_MADCTL_BGR
...
static struct GC9A01_function GC9A01_cfg_script[] = {
	...
	{ GC9A01_DATA, GC9A01_MADCTL_MX | MADCTL_BITORDER }, 
...
void GC9A01_setRotation(uint8_t m) {
  ...
  switch (m) {
   case 0:
     GC9A01_data(GC9A01_MADCTL_MX | GC9A01_MADCTL_MY | MADCTL_BITORDER);

    //  _xstart = _colstart;
    //  _ystart = _rowstart;
     break;
   case 1:
     GC9A01_data(GC9A01_MADCTL_MY | GC9A01_MADCTL_MV | MADCTL_BITORDER);

    //  _ystart = _colstart;
    //  _xstart = _rowstart;
     break;
  case 2:
     GC9A01_data(MADCTL_BITORDER);
   ...
   case 3:
     GC9A01_data(GC9A01_MADCTL_MX | GC9A01_MADCTL_MV | MADCTL_BITORDER);
